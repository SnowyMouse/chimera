...
namespace Chimera {
    ...
    void enable_block_update_check() noexcept {
        ...
        if(chimera.feature_present("client_full")) {
            static Hook hook;
            auto *check_for_update_fn_call_sig = get_chimera().get_signature("check_for_update_fn_call_sig").data();
        ...
        auto *update_already_checked_sig = get_chimera().get_signature("update_already_checked_sig").data();
